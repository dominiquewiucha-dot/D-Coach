...
assert(app.includes('const APP_VERSION = "pwa-v65";'), "app version must be pwa-v65");
assert(sw.includes('const CACHE_NAME = "dcoach-pwa-v65";'), "service worker cache must be pwa-v65");
assert(html.includes("app.js?v=pwa-v65"), "index must load pwa-v65 app bundle");
assert(html.includes("styles.css?v=pwa-v65"), "index must load pwa-v65 css");
...
assert(app.includes("createDailyCheckinDraft(\"training\")"), "training flow must open daily check-in first");
