...
assert(app.includes('const APP_CACHE_VERSION = "dcoach-pwa-v96";'), "app cache version must be dcoach-pwa-v96");
assert(sw.includes('const CACHE_NAME = "dcoach-pwa-v96";'), "service worker cache must be pwa-v96");
assert(html.includes("app.js?v=pwa-v96"), "app cache buster must be pwa-v96");
assert(html.includes("styles.css?v=pwa-v96"), "style cache buster must be pwa-v96");
...
const navBlock = css.match(/\.bottom-nav,\r?\n\.tabs\s*\{[^}]*\}/s)?.[0] || "";
